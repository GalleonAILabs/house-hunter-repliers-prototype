...
- Realtor input must never silently read as buyer sentiment. Every realtor chip (`people[].role === "realtor"`) carries a `.chip-realtor` dashed border plus a hollow-diamond icon prefix, independent of its reject/rated/research/none state colour, so realtor and buyer chips are never visually confusable even at a glance. (`"realtor"` is the single source of truth: the stored `people.role` value, the API value, and the CSS class name all say realtor. An earlier `"advisor"` stored token was renamed by a schema migration, see `migrate_role_advisor_to_realtor`.)
...
- **Highway distance (km) Min/Max** (`#minHwyKm`/`#maxHwyKm`): a plain range filter on the precomputed straight-line `highwayKm`, same `matchesRangeDirect` pattern as Sqft. Independent of the household highway minimum in settings, which is unchanged and still drives the card badge. Blank means no constraint. Replaced the earlier single "Far enough from highways" checkbox (a persisted-on checkbox migrates once to `minHwyKm` = the household `highway_km`, since the checkbox kept `highwayKm >= household_km`).
- **Drive to attached place (min) Min/Max** (`#minAttachDrive`/`#maxAttachDrive`): filters on the cached street-routed drive time of a listing's place attachments; a listing passes if at least one attachment's `drive_minutes` falls in range. **This filter inherently acts on the attached subset:** when either field has a value, listings with no attachments (or none with a computed drive time in range) are excluded, because the filter can only evaluate listings that have one. Both blank = inactive, everything passes. Labelled "Drive to attached place (min)" so the attached-subset scope reads clearly.
- Both new range filters persist across reloads (`PERSISTED_FIELD_IDS`), are cleared by Reset, and apply identically to List and Map via the same `filterByFeedback()`-filtered `state.listings`.
